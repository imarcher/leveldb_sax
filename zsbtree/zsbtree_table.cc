...
void zsbtree_table::Rebalance(int tmp_leaf_maxnum, int tmp_leaf_minnum, int Nt) {
  ...
  root = build_tree_from_leaf(new_sortleafKeys, tmp_leaf_maxnum, tmp_leaf_minnum);
}
...
void zsbtree_table::Rebalance_dfs(NonLeaf* nonLeaf, vector<LeafKey>& sortleafKeys) {
  ...
  if (nonLeaf->isleaf) {
    ...
    for(int i=0;i<nonLeaf->num;i++) {
      ...
      tmpleaf->sort();
      mempcpy(sortleafKeys.data() + sortleafKeys.size(), tmpleaf->leafKeys, leaf_key_size * tmpleaf->num);
      free(tmpleaf);
    }
  }
  else {
